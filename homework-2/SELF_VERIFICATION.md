...
| Docs: README, API_REFERENCE, ARCHITECTURE, TESTING_GUIDE | ✅ | all present; ≥3 Mermaid (5 total); cURL per endpoint; per-doc model note |
...
- **`docs/screenshots/test_coverage.png`** — a screenshot cannot be produced by the agent. A text report is at `docs/screenshots/coverage_report.txt` and an HTML report is generated by `pytest --cov-report=html` (`htmlcov/index.html`). Capture the PNG from either.
